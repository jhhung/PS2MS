...
def train():
    spec = load_npz('DeepEI/data/paper/nist_swg_train_spec.npz')
    fps = load_npz('DeepEI/data/paper/nist_swg_train_fingerprint.npz')
    ...
    # the fp positions to train are known, so read them from a list
    for i, fp_idx in enumerate(fp_list):
        ...
        mlp_result = open('saved/model/paper/nist_swg/mlp_result.txt', 'a+')
        mlp = MLP(spec, Y, spec_test, Y_test)
        history = mlp.train()
        mlp_res = mlp.test()
        mlp_result.write("\t".join([str(fp_idx)] + [str(j) for j in mlp_res]))
        mlp_result.write("\n")
        mlp.save('saved/model/paper/nist_swg/weight/{}.h5'.format(fp_idx), 'saved/model/paper/nist_swg/model.json')
        with open('saved/model/paper/nist_swg/history/{}.history'.format(fp_idx), 'wb') as file_pi:
