...
std::tuple<int, double, std::string, double> compare_candidate(std::string& database_filename, std::string& name, Spectrum& answer, Spectrum& predicted, DotProduct* comparator)
{
	...
	double weight = RDKit::Descriptors::calcExactMW(*mol);
	int rank = 1;
	...
	while (true)
	{
		...
		if (!candidate_mol)
			std::cout << "ptr=0: " << candidate.first << std::endl;
		else if (std::abs(weight - RDKit::Descriptors::calcExactMW(*candidate_mol)) <= 5)
			continue;
...
int main(int argc, char *argv[])
{
	...
	std::string database_filename = "DMPNN_mainlib_only_pred_B.msp";
