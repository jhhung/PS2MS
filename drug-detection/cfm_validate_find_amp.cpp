...
std::pair<int, double> compare_candidate(std::string& database_filename, std::string& name, Spectrum& answer, Spectrum& predicted, DotProduct* comparator)
{
	double score;
	std::string max_name;
	std::ifstream candidates("mainlib_smiles.msp"); // hacked
...
int main(int argc, char *argv[])
{
	...
	std::string msp_filename = "mainlib_peak.msp";
	...
	std::string line, upper_line, smiles, name, sdf_name, synon;
	...
	std::ofstream outfile("mainlib_amp.txt");
	while (getline(msp_library, line) && !quit.load())
	{
		if (line.size() < 3)
		{
			...
			else if (data)
			{
				...
				std::cout << name << ":" << smiles << std::endl;
				outfile << name << ":" << smiles << std::endl;
			}
		...
		if (!ignore)
		{
			...
			{
				name = line.substr(6);
				if (name.find("amphetamine") == std::string::npos)
					name = "";
			}
			else if (upper_line.substr(0, 5) == "SYNON")
			{
				synon = line.substr(7);
				if (name.size() == 0 && synon.find("amphetamine") != std::string::npos)
					name = synon;
			}
			// if (name == "CC")
			// {
