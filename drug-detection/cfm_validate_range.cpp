...
std::vector<int> calc_atom_num(RDKit::RWMOL_SPTR mol, const std::vector<int>& target_atoms)
{
	int atom_num;
	std::vector<int> result(target_atoms.size(), 0);
	for (auto it = mol->beginAtoms(); it != mol->endAtoms(); ++it)
	{
		atom_num = (*it)->getAtomicNum();
		for (int i = 0; i < target_atoms.size(); ++i)
		{
			if (atom_num == target_atoms[i])
			{
				++result[i];
				break;
			}
...
std::tuple<int, double, std::string, double> compare_candidate(std::string& database_filename, std::string& name, Spectrum& answer, Spectrum& predicted, DotProduct* comparator, std::vector<int> target_atoms)
{
	...
	std::vector<int> target_atom_num = calc_atom_num(mol, target_atoms);
	int rank = 1;
	auto equal_function = [](int i, int j) { if (std::abs(i - j) < 2) return true; return false; };
	...
	while (true)
	{
		...
		auto current_atom_num = calc_atom_num(candidate_mol, target_atoms);
		if (!std::equal(target_atom_num.begin(), target_atom_num.end(), current_atom_num.begin()))//, equal_function))
			continue;
...
std::mutex mut;
void predict_and_record(boost::shared_ptr<MolData> data, config_t* cfg, NNParam* nn_param, DotProduct* comparator, std::string name, double prob_thresh_for_prune, int do_annotate, int apply_postprocessing, int suppress_exceptions, std::string database_filename, std::vector<int> target_atoms)
{
	/*
	Param *param;
	...
	*/
    ...
    auto r = compare_candidate(database_filename, name, s, t, comparator, target_atoms);
...
int main(int argc, char *argv[])
{
	...
	for (int i = 3; i < argc; ++i)
		target_atoms.emplace_back(std::stoi(argv[i]));
	...
	while (getline(msp_library, line) && !quit.load())
	{
		if (line.size() < 3)
		{
			...
			else if (data)
			{
				...
				thread_pool.submit(0, std::bind(predict_and_record, data, &cfg, nn_param, &comparator, name, prob_thresh_for_prune, do_annotate, apply_postprocessing, suppress_exceptions, database_filename, target_atoms));
			}
