...
def make_estimator_and_inputs(run_config,
  ...
  #print(estimator.model_fn.eval_matric_ops)

  train_spec = tf.estimator.TrainSpec(train_input_fn, max_steps=train_steps)
  #eval_spec = tf.estimator.EvalSpec(eval_input_fn, steps=None, throttle_secs=0)
...
def main(_):
  ...
  #        save_summary_steps=500, save_checkpoints_steps=500)
  config = tf.contrib.learn.RunConfig(model_dir=FLAGS.model_dir)
