...
class MassSpectraPrediction(object):
  ...
  def features_to_load(self, hparams):
    ...
    feature_names = [
        ...
        fmap_constants.INCHIKEY, fmap_constants.MASK
    ]
  ...
  def make_prediction_ops(self, feature_dict, hparams, mode, reuse=False):
    ...
    with tf.variable_scope('spectrum_predictor', reuse=reuse):
      ...
      if hparams.mask:
        final_prediction = tf.multiply(final_prediction, feature_dict[fmap_constants.MASK])
        prediction_for_loss = tf.multiply(prediction_for_loss, feature_dict[fmap_constants.MASK])
      if hparams.normalize:
        peak_sum = tf.reduce_sum(final_prediction, axis=1, keepdims=True)
        final_prediction = tf.divide(final_prediction, peak_sum) * 100
  ...
  def make_loss(self, pred_val, feature_dict, hparams):
    ...
    elif hparams.loss == 'generalized_mse_add_kl':
      return similarity_lib.GeneralizedCosineSimilarityProvider(
          hparams).make_training_loss(true_spectra, pred_val, add_kl=True)
