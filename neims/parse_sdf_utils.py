...
def make_mol_dict(mol,
  ...
  mask_vec = feature_utils.make_mask(
      mass_spec_locs, max_mass_spec_peak_loc)
...
def dict_to_tfexample(mol_dict):
  ...
  feature_map[fmap_constants.MASK].float_list.value.extend(
      mol_dict[fmap_constants.MASK])
...
def write_dicts_to_example(mol_list,
  ...
  with tf.python_io.TFRecordWriter(record_path_name, options) as writer:
    for idx, mol in enumerate(mol_list):
      ...
      # print(example.features.feature[fmap_constants.MASK])
