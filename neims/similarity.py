...
class GeneralizedCosineSimilarityProvider(CosineSimilarityProvider):
  ...
  def make_training_loss(self, true_tensor, predicted_tensor, add_kl=False):
    if self.hparams.mass_power != 0:
      weights = self._make_weights(true_tensor)
      weighted_squared_error = weights * tf.square(true_tensor -
                                                   predicted_tensor)
      if add_kl:
        weighted_squared_error = tf.reduce_mean(
          tf.concat([
            tf.reduce_mean(weighted_squared_error, -1),
            tf.keras.losses.kullback_leibler_divergence(true_tensor, predicted_tensor)]
          , -1)
        , -1)
      return tf.reduce_mean(weighted_squared_error)
    else:
      mse = tf.reduce_mean(tf.square(true_tensor - predicted_tensor), -1)
      if add_kl:
        normalized_true = tf.divide(true_tensor, tf.reduce_sum(true_tensor, axis=-1, keepdims=True))
        normalized_pred = tf.divide(predicted_tensor, tf.reduce_sum(predicted_tensor, axis=-1, keepdims=True))
        mse = tf.reduce_mean(
          tf.stack([mse, tf.keras.losses.kullback_leibler_divergence(normalized_true, normalized_pred)], -1),
          -1
        )
