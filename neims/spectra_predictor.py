...
def get_mol_list_from_sdf(sdf_fname, hparams):
  ...
  for idx, mol in enumerate(suppl):
    if mol is not None:
      if not hparams.mask or mol.HasProp(ms_constants.SDF_TAG_MASS_SPEC_PEAKS):
        mols.append(mol)
    else:
...
def get_mol_list_from_smiles(smi_fname):
  mols = []
  infile = open(smi_fname, 'r')
  ...
  while line:
    ...
    if mol is not None:
      ...
      raise ValueError("Unable to parse the following smiles at line {0}: {1}".format(line_num, line))
    line = infile.readline().strip()
    line_num += 1
  return mols

def get_mol_list_from_csv(csv_fname):
  mols = []
  infile = open(csv_fname, 'r')
  line = infile.readline().strip()
  line = infile.readline().strip()
  ...
  while line:
    ...
    peaks = []
    for i, intensity in enumerate(line[1:]):
        if len(intensity) != 0:
            peaks.append("{0} {1}".format(i + 1, intensity))
    if mol is not None:
      mol.SetProp(ms_constants.SDF_TAG_MASS_SPEC_PEAKS, "\n".join(peaks))
      mols.append(mol)
    else:
      raise ValueError("Unable to parse the following smiles at line {0}: {1}".format(line_num, line))
    line = infile.readline().strip()
...
def update_mols_with_spectra(mol_list, spectra_array):
  ...
  remove_mol_idx = []
  for i, (mol, spectrum) in enumerate(zip(mol_list, spectra_array)):
    if np.isnan(spectrum).any():
      remove_mol_idx.append(i)
    else:
  ...
  for i in reversed(remove_mol_idx):
    del mol_list[i]
  return mol_list
...
class SpectraPredictor(object):
  ...
  def __init__(self, hparams_str=""):
    ...
    self.mask_key = fmap_constants.MASK
    ...
    assert set(self._placeholder_dict) == set(
        [self.fingerprint_input_key, self.molecular_weight_key, self.mask_key])
  ...
  def make_spectra_prediction(self, fingerprint_array, molecule_weight_array, mask_array):
    ...
    with self._graph.as_default():
      prediction = self._sess.run(
          ...
          feed_dict={
              ...
              self._placeholder_dict[self.mask_key]:
                  mask_array
          })
    if not self._hparams.normalize:
  ...
  def get_mask_from_mol_list(self, mol_list):
    masks = []
    for i, mol in enumerate(mol_list):
      if mol.HasProp(ms_constants.SDF_TAG_MASS_SPEC_PEAKS):
        mask = np.zeros(self._hparams.max_mass_spec_peak_loc)
        mass_spec_locs, mass_spec_intensities = feature_utils.parse_peaks(
          mol.GetProp(ms_constants.SDF_TAG_MASS_SPEC_PEAKS))
        mask[[item for item in mass_spec_locs if item < self._hparams.max_mass_spec_peak_loc]] = 1
      else:
        mask = np.ones(self._hparams.max_mass_spec_peak_loc)
      masks.append(mask)
  ...
  def get_inputs_for_model_from_mol_list(self, mol_list):
    ...
    masks = self.get_mask_from_mol_list(mol_list)
    return fingerprints, weights, masks


class NeimsSpectraPredictor(SpectraPredictor):
  """Helper for making spectra predictions using the trained NEIMS model."""

  def __init__(self, model_checkpoint_dir, hparams_str=""):
  ...
  def _setup_prediction_op(self):
    ...
    mask_input_op = tf.placeholder(tf.float32, (None, self._hparams.max_mass_spec_peak_loc))

    feature_dict = {
        self.fingerprint_input_key: fingerprint_input_op,
        self.molecular_weight_key: mol_weight_input_op,
        self.mask_key: mask_input_op
    }
    ...
    with self._graph.as_default():
      if model_checkpoint_dir:
        ...
        saver.restore(self._sess,
                      ...
                      #"prediction_neims_mse_loss_samuel/models/output/model.ckpt-8701")
      else:
